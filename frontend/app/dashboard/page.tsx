...
export default async function DashboardPage({ searchParams }: Props) {
  ...
  const showWelcome = !statsError && s.gemCount === 0 && s.parcelCount === 0 && s.purchaseOrderCount === 0;
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      ...
      {statsError && (
        <div className="shrink-0 flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
      ...
      {showWelcome && (
        <div className="shrink-0 relative overflow-hidden rounded-xl border border-violet-200/60 bg-gradient-to-br from-violet-50 to-white p-5">
...
function MiniStat({ label, value, icon, href }: { label: string; value: number; icon: ReactNode; href: string }) {
  return (
    <Link href={href} className="flex items-center gap-3 rounded-xl border border-zinc-200/80 bg-card px-4 py-2.5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] transition-shadow hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)]">
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground">{icon}</div>
      <div>
