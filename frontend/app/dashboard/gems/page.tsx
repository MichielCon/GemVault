...
export default async function GemsPage({ searchParams }: Props) {
  ...
  let result;
  try {
    result = await gemsApi.list(page, pageSize, search, undefined, status);
  } catch {
    return <p className="text-muted-foreground">Failed to load gems. Is the API running?</p>;
  }

  return <GemInventoryView result={result} page={page} pageSize={pageSize} search={search} status={status} />;
}
